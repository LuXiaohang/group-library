...
<div id="page" style="display: none;text-align:center;margin:auto;">
 <input type="text" name="page" class="search_key" value="全文检索" id=textselectp onmouseover="javascript:selectInputContent(this.id)"  > 
 <input type="submit" class="submit" value="书海漫游">
</div>
...
<h2><font color="black"><center>检索结果</center></font></h2>
   ...
   if($keyword!=null and $keyword!="主题检索"){  
	  ...
	  $sql="SELECT  count(*) FROM `book-information` WHERE abstract like '%$pages%';";
      }
	//echo $sql;
	...
	if($keyword!=null and $keyword!="主题检索"){
	  ...
	  $sql="SELECT  `callnumber`, `name`, `firstauthor`,  `subject1`, `subject2`,`subject3`,`language`, `abstract` FROM `book-information` WHERE abstract like '%$pages%' order by name limit {$offset}, {$length}";
      }
	//echo $sql;
    $result = mysqli_query($con,$sql);
